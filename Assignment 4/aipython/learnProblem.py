...
class Data_set(Displayable):
    ...
    def __init__(self, train, test=None, prob_test=0.30, target_index=0, header=None):
        ...
        self.train = train
        self.test = test
        self.display(1,f"Training set has {len(train)} examples. Number of columns in ",{len(e) for e in train})
        self.display(1,f"Test set has {len(test)} examples. Number of columns in ",{len(e) for e in test})
    ...
    def evaluate_dataset(self, data, predictor, error_measure):
        ...
        if data:
            try:
                error = mean(error_measure(predictor(e), self.target(e)) 
                            for e in data)
            except ValueError:
...
def absolute_error(prediction, actual):
    "absolute error"
...
def log_loss(prediction,actual):
    ...
    try: 
      if actual==0:
        return -math.log2(1-prediction)
      else:
        return -math.log2(prediction)
    except ValueError:
        return float("inf")  # infinity 

evaluation_criteria = [squared_error, absolute_error, log_loss]

def mean(enum, isum=0, icount=0):
    """returns the mean of enumeration enum, 
       isum is the initial sum, and icount is the initial count."""
    for e in enum:
        icount += 1
        isum += e
    return isum/icount
